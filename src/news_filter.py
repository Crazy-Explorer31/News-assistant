...
def get_filtered_news(
    ...
    interesting_categories = [category_code for category, category_code in categories.items() if choosen_categories[category]]
    
    news_classified = classifier.predict(news_df).tolist()
    print(news_classified)

    news_interesting_list = [news for i, news in enumerate(news_list) if news_classified[i] in interesting_categories]
