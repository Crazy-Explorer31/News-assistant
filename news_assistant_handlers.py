...
def get_keyboard():
    keyboard = [
        ...
        ["Просмотреть каналы", "Получить новости"]
    ]
...
async def add_channels(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Введите каналы через пробел:")
...
async def remove_channels(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text("Введите каналы для удаления через пробел:")
...
async def view_channels(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if channels:
        await update.message.reply_text(f'Текущий список каналов: {", ".join(channels)}')
    else:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == 'add':
        ...
        if new_channels:
            ...
            await update.message.reply_text("Вы не ввели каналы.")
        context.user_data['action'] = None  # Сбрасываем действие
    elif action == 'remove':
        channels_to_remove = update.message.text.split()
        removed_channels = [channel for channel in channels_to_remove if channel in channels]
        for channel in removed_channels:
            channels.remove(channel)
        if removed_channels:
            await update.message.reply_text(f'Каналы удалены: {", ".join(removed_channels)}')
        else:
            await update.message.reply_text("Каналы не найдены для удаления.")
        context.user_data['action'] = None  # Сбрасываем действие
    else:
